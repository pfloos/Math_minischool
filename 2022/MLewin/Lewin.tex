...
\end{theorem} 
  ...
  Thm: $H = -\Delta +  V$ with $V$ a $\cL$-periodic pontential and bounded \int_C V^2 < \infty
    ...
    Id_{L^2(\RR^d)} = \frac{1}{|B|} \int_B \sum_n \dyad{\varphi_{n,\xi}} d\xi 
    H = \frac{1}{|B|} \int_B \sum_n \la_n(\xi) \dyad{\varphi_{n,\xi}} d\xi 
    \sig(H) = \bigcup_{n \ge 1} { \la_n(\xi) : \xi \in \bar{B} }
  ...
  Thm: For \bar{\rho} given, \forall f \in L^2(\RR^d), \gamma_L \II_{C_L} f \to \II_{(-\infty,\eps_F]} (-\Delta+V) (this is the spectral projection) 
    ...
    \bar{\rho} = \frac{1}{(\2pi)^d} \int_B \sum_{n \ge 1} \II(\la_n(\xi) \le \eps_F) d\xi
    \bar{\rho} is the integrated density of state.

    \gamma_{\infty} = \frac{1}{|B|} \II_{(-\infty,\eps_F}(-\Delta+V) 
                    = \frac{1}{|B|} \int_B \sum_{n \ge 1} \II(\la_n(\xi) \le \eps_F) \dyad{\varphi_{n,\xi}} d\xi
    $\gamma_{\infty}$ is named the Fermi sea.
    \rho_{\infty}(x) = \frac{1}{|B|} \int_B \sum_{n \ge 1} \II(\la_n(\xi) \le \eps_F) \abs{\varphi_{n,\xi}}^2 d\xi 
    and $\rho_{\infty}(x)$ is \cL-periodic.
    Often, one has an integed number of particle per cell, i.e., K electrons per unit cell
    Therefore, \Bar{\rho} = \frac{K}{|C|}. Therefore, 
    K = 1/|B| \int_B \sum_{n \ge 1} \II(\la_n(\xi) \le \esp_F) d\xi
  ...
  Regularity of \la_n(\xi) and degeneracies.
    ...
    \Tilde{H}(\xi) = -\Delta + V + 2 P.\xi + |\xi|^2
  ...
  Thm: 
    ...
    \II_{[a,b]}(A(\xi))  = \sum_{\la_n(\xi) \in [a,b] \dyad{v_{n,\xi}} has the same regularity as A.
  ...
  \la_n(\xi) = \Tr[A(\xi)\II_{[a,b]}(A(\xi))] is nice!
  
  proof of theorem: 
    A(\xi) - z = A(\xi_0) - z + A(\xi) - A(\xi_0) = (1 + (A(\xi) - A(\xi_0)) (A(\xi_0) - z)^{-1}) (A(\xi_0) - z)
    We know that (A(\xi_0) - z) is invertible for z ~ a and z ~ b.
    We know also that (A(\xi) - A(\xi_0)) is small by continuity arguments.
    Therefore, A(\xi) - z is invertible.
    
    (A(\xi) - z)^{-1} = (A(\xi_0) - z)^{-1} (1 + B)^{-1} (regularuty for series)
    The resolvent is as nice as the operator (where it is defined).
 
  Cauchy's formula: 
  \II_{[a,b]} = \frac{1}{2\pi} \oint_{\cC} \frac{dz}{z-x}
  => \II_{A(\xi)} = - \frac{1}{2\pi} \oint_{\cC} (A(\xi) - z)^{-1} dz
